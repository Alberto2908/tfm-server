...
/**
 * Tarea puntual: recorre de nuevo NVD y actualiza SOLO impact/mitigation de los
 * CVE ya importados (createdBy/verifiedBy = "import:nvd"), con el nuevo formato
 * derivado del vector CVSS real en vez del texto genérico anterior.
 *
 * No inserta nada nuevo (para eso está CveImportRunner). Usa su propio
 * checkpoint ("UPDATE_SWEEP") para poder reanudar sin repetir trabajo.
 */
@Component
@ConditionalOnProperty(name = "update.cve.enabled", havingValue = "true")
public class CveUpdateRunner implements CommandLineRunner {

    private static final Logger log = LoggerFactory.getLogger(CveUpdateRunner.class);
    private static final String CHECKPOINT_KEY = "UPDATE_SWEEP";

    private final MongoTemplate mongoTemplate;
    private final ConfigurableApplicationContext applicationContext;
    private final String nvdApiKey;

    public CveUpdateRunner(
    ...
    public void run(String... args) throws Exception {
        if (nvdApiKey == null || nvdApiKey.isBlank()) {
            log.warn("update.cve.enabled=true pero no se ha definido NVD_API_KEY. Se continúa sin key "
        ...
        log.info("Actualización de impact/mitigation iniciada desde startIndex={}", startIndex);
        ...
        while (!exhausted) {
            ...
            try {
                ...
                log.error("Fallo consultando NVD en startIndex={}, se detiene la actualización", startIndex, e);
                break;
            }
            ...
            if (pageSize > 0) {
                ...
                for (JsonNode item : vulnerabilities) {
                    ...
                    Update update = new Update()
                            .set("impact", mapped.getImpact())
                            .set("mitigation", mapped.getMitigation());
            ...
            log.info("Progreso actualización: {} actualizados hasta ahora (startIndex {}/{})",
                    updatedCount, startIndex, totalResults);
        }

        log.info("Actualización de impact/mitigation finalizada: {} documentos actualizados "
